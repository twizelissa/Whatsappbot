...
exports.generateCallRecap = generateCallRecap;
exports.generateCatchUp = generateCatchUp;
exports.generateMeetingIntelligence = generateMeetingIntelligence;
exports.detectGroupConfusion = detectGroupConfusion;
...
function getAnthropic() {
    if (!_anthropic) {
        ...
        _anthropic = new sdk_1.default({ apiKey: env.ANTHROPIC_API_KEY });
    }
    return _anthropic;
}
function getOpenAI() {
...
function getGemini() {
    if (!_gemini) {
        ...
        _gemini = new generative_ai_1.GoogleGenerativeAI(env.GEMINI_API_KEY);
    }
    return _gemini;
}
...
Please answer the question based on the thread conversation history and context above.`;
    ...
    if (env.LLM_PROVIDER === 'anthropic') {
        ...
        const candidateModels = Array.from(new Set([env.LLM_MODEL, 'gemini-2.5-flash', 'gemini-2.5-pro']));
        let lastError = null;
        let success = false;
        answer = '';
 ...
 */
async function generateDigest(chunks, period) {
    const env = (0, config_1.default)();
    const contextStr = chunks
        .map((c) => {
        const meta = c.metadata;
 ...
 */
async function generateCallRecap(chunks, callId) {
    const env = (0, config_1.default)();
 ...
 */
async function generateCatchUp(timeframeLabel, chunks, userName) {
    const env = (0, config_1.default)();
    const contextStr = chunks
        .map((c) => {
        const meta = c.metadata;
 ...
 */
async function generateMeetingIntelligence(callId, chunks) {
    const env = (0, config_1.default)();
 ...
 */
async function detectGroupConfusion(recentMessages) {
